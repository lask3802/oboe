...
int read_mem(void* opaque, uint8_t* buf, int buf_size){
    auto compressed = reinterpret_cast<ReadOnlyMemoryStream*>(opaque);
    return compressed->read(buf, buf_size);
}
...
int64_t seek(void *opaque, int64_t offset, int whence){
 ...
 int64_t seek_mem(void *opaque, int64_t offset, int whence){
    auto compressed = reinterpret_cast<ReadOnlyMemoryStream*>(opaque);
    if (whence == AVSEEK_SIZE) return compressed->size();
    if (compressed->seek(offset) == -1){
        return -1;
    } else {
        return 0;
    }
...
void FFMpegExtractor::printCodecParameters(AVCodecParameters *params) {
 ...
 bool FFMpegExtractor::createAVIOContext(ReadOnlyMemoryStream &assetData, uint8_t *buffer,
                                         uint32_t bufferSize, AVIOContext **avioContext) {
     constexpr int isBufferWriteable = 0;
     ...
     *avioContext = avio_alloc_context(
             ...
             bufferSize, // For optimal decoding speed this should be the protocol block size
             isBufferWriteable,
             &assetData, // Will be passed to our callback functions as a (void *)
             read_mem, // Read callback function
             nullptr, // Write callback function (not used)
             seek_mem); // Seek callback function
 ...
 int64_t FFMpegExtractor::decode(const std::vector<uint8_t> &assetData, uint8_t *targetData,
                                 AudioProperties targetProperties) {
     LOGI("Decoder: FFMpeg");
     ...
     {
         AVIOContext *tmp = nullptr;
         auto sharedStream = SharedStream<decltype(assetData)>(assetData);
         if (!createAVIOContext(sharedStream, buffer, kInternalBufferSize, &tmp)){
             LOGE("Could not create an AVIOContext");
             return returnValue;
         }
         ioContext.reset(tmp);
